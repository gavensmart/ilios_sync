...
SYNCED_CALENDAR_ID  = "85f197b53071c74c81735c049139e5abf2c44044a07d740ff78f3450a2276029@group.calendar.google.com"
...
def mirror_and_color():
    ...
    # Try to load previously mirrored event keys
    mirrored_uids_path = MIRRORED_JSON_PATH
    if os.path.exists(mirrored_uids_path):
        with open(mirrored_uids_path, "r") as f:
    ...
    # Sort keyword map by length so specific matches (like "anatomy lab quiz") come before "anatomy"
    ...
    for evt in events:
        ...
        try:
            ...
            print(f"✅ Added: {evt['summary']} → color {color_id or 'default'}")
            added += 1
            time.sleep(0.3)
        except Exception as e:
            print(f"❌ Failed to add {evt['summary']}: {e}")

    # Save updated mirrored_uids
    with open(mirrored_uids_path, "w") as f:
