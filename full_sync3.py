...
PUBLIC_CALENDAR_ID  = "77b26317c6f746eba17e9fbcf9ee9edf12cc4bc953e4be26f50163675af967f4@group.calendar.google.com"
PRIVATE_CALENDAR_ID = "279b20af2b59ce5879fc0494c8963b325c0936767898e71ef77a3dcca9441bfb@group.calendar.google.com"
...
def mirror_and_color():
    ...
    for evt in events:
        ...
        # Build the event body once
        description = (evt.get("description", "").strip() + f"\nUID:{evt['uid']}").strip()
        combo = (evt["summary"] + " " + evt.get("description", "")).lower()

# 1) Explicit override for anatomy lab quiz
    if "anatomy lab quiz" in combo:
        color_id = "7"   # your light-blue quiz color
    else:
        # 2) Fallback to your existing longest-keyword match
        ...
        # Insert/update into each calendar
    ...
    # Persist each mirrored-UID set back to its file
    for cal_id, mirror_path in [
        (PRIVATE_CALENDAR_ID, MIRRORED_JSON_PRIVATE),
        (PUBLIC_CALENDAR_ID,  MIRRORED_JSON_PUBLIC)
    ]:
        with open(mirror_path, "w") as f:
            json.dump(list(mirrored[cal_id]), f, indent=2)
