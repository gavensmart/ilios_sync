...
def mirror_and_color():
    ...
    # Load mirrored events (safe fallback if file doesn't exist)
    mirrored = set()
    try:
    ...
    print(f"DEBUG ▶️ pre-run mirrored count: {len(mirrored)}")

    # Parse events from latest .ics file
    events = parse_ics("ilios_latest.ics")
    ...
    for evt in events:
        ...
        # Match color keyword
        color = next((c for k, c in sorted_keywords if k in combo), None)
        ...
        try:
            ...
            mirrored.add(key)
            print(f"✅ Added: {evt['summary']} → color {color or 'default'}")
            added += 1
            time.sleep(0.3)  # Respect Google API rate limits
