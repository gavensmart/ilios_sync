...
SYNCED_CALENDAR_ID  = "a2e26fbe3c958f6f21d5abad3dd45186c5250ca3af9c39cb88679fde3f4510e9@group.calendar.google.com"
...
def mirror_and_color():
    ...
    # Load stored events
    events = json.load(open(STORED_JSON_PATH))

    # Debug: show we’re in the right folder and loading the right JSON
    print("DEBUG ▶️ cwd:", ROOT)
    print("DEBUG ▶️ mirrored JSON exists?", os.path.exists(MIRRORED_JSON_PATH))
    try:
        mirrored = set(json.load(open(MIRRORED_JSON_PATH)))
    except Exception as e:
        print("⚠️ Could not load mirrored_uids.json:", e)
        mirrored = set()
    print("DEBUG ▶️ pre-run mirrored count:", len(mirrored))
