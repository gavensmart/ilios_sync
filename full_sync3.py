...
PUBLIC_CALENDAR_ID  = "923ec3bb3b4094b93e656b4219d7cdd6b8b6afd3276884fa59d86b03179e2efa@group.calendar.google.com"
PRIVATE_CALENDAR_ID = "38cb3817ad0072fde216eccd7c98713284104e8c68dafcc6d11870a63be70378@group.calendar.google.com"
...
def mirror_and_color():
    ...
    # Load mirrored_uids separately for each calendar
    mirrored_uids = {}
    for cal_id in [PRIVATE_CALENDAR_ID, PUBLIC_CALENDAR_ID]:
        path = f"mirrored_uids_{cal_id.split('@')[0]}.json"
        if os.path.exists(path):
            with open(path, "r") as f:
                mirrored_uids[cal_id] = set(json.load(f))
        else:
            print(f"⚠️ No UID file for {cal_id}, starting fresh...")
            mirrored_uids[cal_id] = set()

    # Sort keywords by length (more specific first)
    ...
    for evt in events:
        ...
        combined_text = (evt["summary"] + " " + evt.get("description", "")).lower()
        ...
        for cal_id in [PRIVATE_CALENDAR_ID, PUBLIC_CALENDAR_ID]:
            if key in mirrored_uids[cal_id]:
                continue
            try:
                service.events().insert(calendarId=cal_id, body=body).execute()
                mirrored_uids[cal_id].add(key)
                print(f"✅ Added to {cal_id}: {evt['summary']} → color {color_id or 'default'}")
                time.sleep(0.3)
            except Exception as e:
                print(f"❌ Failed to add to {cal_id}: {evt['summary']} - {e}")

    # Save mirrored UID sets
    for cal_id in [PRIVATE_CALENDAR_ID, PUBLIC_CALENDAR_ID]:
        path = f"mirrored_uids_{cal_id.split('@')[0]}.json"
        with open(path, "w") as f:
            json.dump(list(mirrored_uids[cal_id]), f, indent=2)
